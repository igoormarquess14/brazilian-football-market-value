...
**2026-09-07 update:** the squashed commit was originally pushed to `master`. A PR from branch `rename-2024-to-2025` (2024→2025 naming cleanup, see below) was renamed to `master-25` on GitHub and set as the repository's default branch; `master` was then confirmed to contain no commits absent from `master-25` and deleted, both locally and on the remote. The current default and only branch is `master-25`.

**2026-09-07 — 2024→2025 naming cleanup:** the `_2024` suffix in `codes/pipeline/01`, `02`, `03`, `05` and their intermediate `todos_clubes_*` filenames was a leftover from the first data-collection attempt; the sample is the 2025 Série A. Renamed to `todos_clubes_2025*`; `01`'s club-name derivation now uses a regex matching any 4-digit year suffix, so it keeps working against local raw dumps still named `_2024`. `codes/scraping/codigo_ok.py`'s `__main__` block bumped to the 2025 season URL/output name. `.gitignore` consolidated `/dados/clubes/*_2024*.csv` + `/dados/clubes/*_2024_table_only.csv` into a single `/dados/clubes/*_20??*.csv`. `README.md` and `docs/data_sources.md` updated to match and note that a local `_2024` filename suffix is a legacy convention, not the season. Verified before merging: the renamed 01→02→03 chain reproduces `todos_clubes_2025_final.csv` byte for byte against the pre-rename `todos_clubes_2024_final.csv`, and `codes/pipeline/13`'s model output is unchanged.

Also removed the `Co-Authored-By: Claude` and `Claude-Session` trailers from this branch's commit history (rewritten and force-pushed) at the user's request, and set `includeCoAuthoredBy: false` / `attribution.sessionUrl: false` in `~/.claude/settings.json` so future commits don't get them.
